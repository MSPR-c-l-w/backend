...
export class ExerciceService implements IExerciceService {
    ...
    private readonly SOURCE_URL = 'https://raw.githubusercontent.com/yuhonas/free-exercise-db/main/dist/exercises.json';
    private readonly IMG_BASE_URL = 'https://raw.githubusercontent.com/yuhonas/free-exercise-db/main/exercises/';

    private readonly dictionary = {
        'abdominals': 'abdominaux', 'hamstrings': 'ischio-jambiers', 'adductors': 'adducteurs',
        'quadriceps': 'quadriceps', 'biceps': 'biceps', 'shoulders': 'épaules',
        'chest': 'pectoraux', 'middle back': 'milieu du dos', 'lower back': 'lombaires',
        'lats': 'dorsaux', 'triceps': 'triceps', 'traps': 'trapèzes', 'forearms': 'avant-bras',
        'glutes': 'fessiers', 'calves': 'mollets', 'neck': 'cou', 'abductors': 'abducteurs',
        'body only': 'poids du corps', 'dumbbell': 'haltères', 'barbell': 'barre',
        'cable': 'poulie', 'machine': 'machine', 'kettlebells': 'kettlebells',
        'beginner': 'débutant', 'intermediate': 'intermédiaire', 'expert': 'expert',
        'pull': 'tirage', 'push': 'poussée', 'static': 'statique',
        'compound': 'polyarticulaire', 'isolation': 'isolation',
        'strength': 'force', 'stretching': 'étirement', 'cardio': 'cardio'
    };
    ...
    async getExercices(page: number = 1, limit: number = 20): Promise<Exercise[]> {
        ...
        if (exercices.length === 0) throw new NotFoundException('Aucun exercice trouvé');
        return exercices;
    }
    ...
    async runImportPipeline(): Promise<number> {
        try {
            ...
            const response = await lastValueFrom(this.httpService.get(this.SOURCE_URL));
            ...
            for (let i = 0; i < rawData.length; i += BATCH_SIZE) {
                ...
                const allInstructionsStrings = batch.map(item => (item.instructions || []).join(' ||| '));

                let translatedBatch: string[] = [];
                try {
                    const res: any = await translate(allInstructionsStrings, { to: 'fr' });
                    translatedBatch = Array.isArray(res) ? res.map((r: any) => r.text) : [res.text];
                } catch (e) {
                ...
                const promises = batch.map((item, index) => {
                    ...
                    return this.prisma.exercise.upsert({
                        ...
                        update: {
                            primary_muscles: (item.primaryMuscles || []).map((m: string) => this.translateTerm(m)),
                            secondary_muscles: (item.secondaryMuscles || []).map((m: string) => this.translateTerm(m)),
                ...
                this.logger.log(`Statut : ${successCount}/${rawData.length} synchronisés.`);
    ...
    async findByFilters(filters: GetExercisesFilterDto): Promise<Exercise[]> {
        const { muscle, level, equipment, category, page = 1, limit = 20} = filters;
        const skip = (page - 1) * limit;
