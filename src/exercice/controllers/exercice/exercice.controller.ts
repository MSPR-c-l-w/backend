...
import { ApiBadGatewayResponse, ApiInternalServerErrorResponse, ApiNotFoundResponse, ApiOkResponse, ApiOperation, ApiParam, ApiQuery, ApiTags } from '@nestjs/swagger';
...
export class ExerciceController implements IExerciceController {
    ...
    @Post('import')
    @ApiOperation({ summary: 'Lancer manuellement la collecte et transformation des données (ETL)' })
    @ApiOkResponse({ description: 'Collecte et synchronisation réussies' })
    @ApiInternalServerErrorResponse({ description: 'Erreur lors du pipeline ETL' })
    ...
    @ApiQuery({ name: 'muscle', required: false, description: 'Ex: pectoraux, biceps...' })
    @ApiQuery({ name: 'level', required: false, description: 'Ex: débutant, expert...' })
    @ApiQuery({ name: 'equipment', required: false, description: 'Ex: haltères, barre...' })
    @ApiQuery({ name: 'category', required: false, description: 'Ex: force, cardio...' })
    async search(
    ...
    @Get(':id')
    async getExerciceById(@Param('id', ParseIntPipe) id: number): Promise<Exercise> {
