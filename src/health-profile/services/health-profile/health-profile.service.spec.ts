...
describe('HealthProfileService', () => {
  ...
  describe('getHealthProfiles', () => {
    it('returns health profiles when found', async () => {
      ...
      await expect(service.getHealthProfiles()).resolves.toEqual(healthProfiles);
    ...
    it('throws NotFoundException when none found', async () => {
      ...
      await expect(service.getHealthProfiles()).rejects.toBeInstanceOf(NotFoundException);
    });
  });
  ...
  describe('getHealthProfile', () => {
    it('returns health profile when found and parses id', async () => {
      ...
      await expect(service.getHealthProfile('42')).resolves.toEqual(healthProfile);
    ...
    it('throws when not found', async () => {
      ...
      await expect(service.getHealthProfile('42')).rejects.toBeInstanceOf(Error);
    });
  });
});
