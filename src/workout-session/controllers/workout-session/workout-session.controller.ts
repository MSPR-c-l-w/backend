...
import type { IWorkout_SessionService, IWorkout_SessionController } from 'src/workout-session/interfaces/workout-session/workout-session.interface';
...
export class Workout_SessionController implements IWorkout_SessionController {
  ...
  @ApiOkResponse({ description: 'Retourne le total des calories, heures et sessions' })
  ...
  @ApiOkResponse({ description: 'Retourne le niveau (Légende, Athlète, etc.) basé sur les calories' })
  ...
  @ApiOkResponse({ description: 'Retourne les moyennes de BPM et calories par session' })
  ...
  @Get('history/:userId')
  @ApiOperation({ summary: 'Récupérer l\'historique complet des séances d\'un utilisateur' })
  @ApiOkResponse({ description: 'Liste des sessions incluant les détails des exercices' })
  ...
  @ApiOkResponse({ description: 'Détails de la séance avec ses logs d\'exercices' })
