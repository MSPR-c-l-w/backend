...
export class Exercise_LogController implements IExercise_LogController {
  ...
  @Post('import')
  @ApiOperation({ summary: 'Déclencher la pipeline ETL pour importer les données Kaggle (Sessions + Logs)' })
  ...
  @ApiOperation({ summary: 'Obtenir les 5 exercices les plus populaires (tous utilisateurs confondus)' })
  ...
  @Get()
  @ApiOperation({ summary: 'Récupérer l\'intégralité des logs d\'exercices (détails techniques)' })
  async getExerciseLogs(): Promise<any[]> {
    return await this.exerciseLogService.getExerciseLogs();
  }

  @Get(':id')
  @ApiOperation({ summary: 'Récupérer un log d\'exercice spécifique par son ID' })
  @ApiNotFoundResponse({ description: 'Log introuvable' })
  async getExerciseLogById(@Param('id', ParseIntPipe) id: number): Promise<any> {
