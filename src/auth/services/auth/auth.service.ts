...
import { generateResetToken, hashResetToken } from 'src/utils/security/reset-token';
...
export class AuthService {
  ...
  private async rotateRefreshToken(refreshToken: string) {
    ...
    const access_token = await this.signAccessToken({ id: user.id, email: user.email });
  ...
  async register(dto: RegisterDto): Promise<{ message: string; token?: string }> {
    ...
    const token = await this.createAndSendEmailVerification(created.id, created.email);
  ...
  async requestAccountVerification(email: string): Promise<{ message: string; token?: string }> {
    const user = await this.prisma.user.findUnique({
      where: { email },
      select: {
        id: true,
  ...
  async confirmAccountVerification(token: string): Promise<{ message: string }> {
    const tokenHash = hashResetToken(token);
