...
export class AuthController {
    ...
    @Post('refresh')
    @ApiOperation({ summary: 'Récupérer un nouvel access token via refresh token (rotation)' })
    ...
    @ApiOperation({ summary: 'Envoyer un email de validation de compte (réponse générique)' })
    ...
    @ApiOperation({ summary: 'Confirmer le reset mot de passe avec token one-time' })
    ...
    @ApiBadRequestResponse({ description: 'INVALID_OR_EXPIRED_TOKEN / validation DTO' })
    ...
    @ApiOperation({ summary: 'Retourner le profil de l’utilisateur connecté (JWT requis)' })
