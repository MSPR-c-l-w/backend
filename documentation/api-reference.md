...
Le champ `media_url` d'un post contient soit l'URL d'un média unique, soit un
tableau JSON d'URLs lorsqu'il y a plusieurs photos/vidéos. Les URLs pointent
vers le bucket S3/MinIO (voir `/media`).
...
| Méthode | Endpoint        | Guards | Description                                                                                                                         |
| ------- | --------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------- |
| POST    | `/media/upload` | 🔒 JWT | Upload `multipart/form-data` de `files[]` (photos & vidéos, max 10) vers le bucket S3/MinIO. Retourne `{ files: [{ url, type }] }`. |

**Limites d'upload** (configurables via `.env`) :

- Images : `MAX_IMAGE_UPLOAD_MB` (défaut 10 Mo) — `image/jpeg|png|webp|gif|heic|heif`
- Vidéos : `MAX_VIDEO_UPLOAD_MB` (défaut 100 Mo) — `video/mp4|quicktime|webm|x-matroska|3gpp`

Erreurs : `413 IMAGE_TOO_LARGE_MAX_xMB` / `VIDEO_TOO_LARGE_MAX_xMB`,
`400 UNSUPPORTED_MEDIA_TYPE:<mimetype>`, `400 NO_FILES_PROVIDED`.
