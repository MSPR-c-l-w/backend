...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **UserAiPreferences**         | <u>**id**</u>, `# user_id`, allergies, regime, budget, objectif_ia, contraintes_materielles, updated_at, _(en cours : limitations_physiques, preferences_sportives — branche `feat/ai-photo-food-detection`)_ | Profil IA stable, 1-1 avec User. Listes en JSON (allergies, contraintes_materielles) car leur contenu varie sans justifier une table de jointure dédiée. `limitations_physiques`/`preferences_sportives` (pas encore mergées) séparent les contraintes sport des contraintes nutrition — corrige un mélange sémantique où `AiWorkoutService` envoyait `regime`/`allergies` (nutrition) comme `preferences`/`limitations` du profil sportif.                         |
...
| Modification d'entités existantes              | _(en cours, branche `feat/ai-photo-food-detection`, pas mergée)_ `UserAiPreferences` += `limitations_physiques` (Json, `@default("[]")`), `preferences_sportives` (Json, `@default("[]")`) — migration `20260610000000_add_limitations_preferences_sportives`. Corrige un bug de mapping où le profil sportif recevait des champs nutrition (`regime`, `allergies`) en guise de préférences/limitations sportives. |
...
| Flux vérifié (code prêt, branches non mergées) | `budget` (`UserAiPreferences`) → `AiNutritionService.generateMealPlanForUser` → `MealPlanRequest.budget` (api-ia, branche `feat/ml-recommendation-model`) : noms de champs et types vérifiés identiques de bout en bout — fonctionnel dès que les deux branches sont mergées, pas encore testé en intégration réelle.                                                                                              |
